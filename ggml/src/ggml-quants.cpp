...
static void quantize_row_iq3_xxs_impl(int grid_size, const float * GGML_RESTRICT x, void * GGML_RESTRICT vy, int64_t n,
    ...
    if (grid_size == 256) {
        ...
        dh = &y->d;
        qs = y->qs;
        blck_size = sizeof(block_iq3_xxs);
    } else {
        block_iq3_s * y;// = vy;
        dh = &y->d;
        qs = y->qs;
        blck_size = sizeof(block_iq3_s);
    }
    int quant_size = blck_size - sizeof(ggml_fp16_t);
    ...
    for (int ibl = 0; ibl < nbl; ++ibl) {
        ...
        if (!max_scale) {
            ...
            dh += blck_size/sizeof(ggml_fp16_t);
            qs += blck_size;
            continue;
        }
...
static void quantize_row_iq3_s_impl(int blck_size, const float * GGML_RESTRICT x, void * GGML_RESTRICT vy, int n,
        const float * GGML_RESTRICT quant_weights,
        float   * scales,
        float   * weight,
        float   * xval,
    ...
    const int bs4 = blck_size/4;
    const int bs8 = blck_size/8;
    ...
    for (int ibl = 0; ibl < nbl; ++ibl) {
        ...
        for (int ib = 0; ib < QK_K/blck_size; ++ib) {
            const float * xb = xbl + blck_size*ib;
            if (quant_weights) {
                const float * qw = quant_weights + QK_K*ibl + blck_size*ib;
                for (int i = 0; i < blck_size; ++i) weight[i] = qw[i] * sqrtf(sigma2 + xb[i]*xb[i]);
            } else {
                for (int i = 0; i < blck_size; ++i) weight[i] = xb[i]*xb[i];
            }
            for (int i = 0; i < blck_size; ++i) waux[i] = sqrtf(weight[i]);
            for (int k = 0; k < bs8; ++k) {
                uint8_t s = 0;
                for (int i = 0; i < 8; ++i) {
                    if (xb[8*k + i] >= 0) xval[8*k + i] = xb[8*k + i];
            ...
            float max = xval[0];
            for (int i = 1; i < blck_size; ++i) max = MAX(max, xval[i]);
            if (!max) {
                scales[ib] = 0;
                continue;
            }
            ...
            for (int is = -9; is <= 9; ++is) {
                ...
                if (sumq2 > 0 && sumqx*sumqx > best*sumq2) {
                    scale = sumqx/sumq2; best = scale*sumqx;
                    for (int i = 0; i < blck_size; ++i) L[i] = Laux[i];
        ...
        float id = 1/d;
        for (int ib = 0; ib < QK_K/blck_size; ib += 2) {
...
static void quantize_row_iq1_s_impl(const float * GGML_RESTRICT x, void * GGML_RESTRICT vy, int64_t n, const float * GGML_RESTRICT quant_weights,
    ...
    const int blck_size = IQ1S_BLOCK_SIZE;
    ...
    for (int ibl = 0; ibl < nbl; ++ibl) {
        ...
        for (int ib = 0; ib < QK_K/blck_size; ++ib) {
            ...
            for (int i = 1; i < blck_size; ++i) max = MAX(max, fabsf(xb[i]));
            if (max < GROUP_MAX_EPS_IQ1_S) {
                scales[ib] = 0;
                memset(L, 1, blck_size);
                continue;
            }
            ...
            qsort(pairs, blck_size, 2*sizeof(float), iq1_sort_helper);
            {
                sumx[0] = sumw[0] = 0;
                for (int j = 0; j < blck_size; ++j) {
                    int i = idx[2*j];
            ...
            for (int i1 = 0; i1 <= blck_size; ++i1) {
                for (int i2 = i1; i2 <= blck_size; ++i2) {
                    float sumqx = (sumx[i1] - sumx[0])*x_p[0] + (sumx[i2] - sumx[i1])*x_p[1] + (sumx[blck_size] - sumx[i2])*x_p[2];
                    float sumq2 = (sumw[i1] - sumw[0])*x_p[0]*x_p[0] + (sumw[i2] - sumw[i1])*x_p[1]*x_p[1] + (sumw[blck_size] - sumw[i2])*x_p[2]*x_p[2];
                    if (sumq2 > 0 && sumqx*sumqx > best_score*sumq2) {
                        scale = sumqx/sumq2; best_score = scale*sumqx;
                        besti1 = i1; besti2 = i2; best_shift = 1;
                    }
                    sumqx = (sumx[i1] - sumx[0])*x_m[0] + (sumx[i2] - sumx[i1])*x_m[1] + (sumx[blck_size] - sumx[i2])*x_m[2];
                    sumq2 = (sumw[i1] - sumw[0])*x_m[0]*x_m[0] + (sumw[i2] - sumw[i1])*x_m[1]*x_m[1] + (sumw[blck_size] - sumw[i2])*x_m[2]*x_m[2];
                    if (sumq2 > 0 && sumqx*sumqx > best_score*sumq2) {
                        scale = sumqx/sumq2; best_score = scale*sumqx;
                        besti1 = i1; besti2 = i2; best_shift = -1;
                    }
            ...
            for (int j = besti2; j < blck_size; ++j) L[idx[2*j]] = 2;
            if (scale < 0) {
                for (int j = 0; j < blck_size; ++j) L[j] = 2 - L[j];
                scale = -scale; best_shift = -best_shift;
            }
            bool all_on_grid = true;
            const float * xx = best_shift == 1 ? x_p : x_m;
            for (int k = 0; k < blck_size/8; ++k) {
                uint16_t u = 0;
            ...
            uint16_t h = 0;
            for (int k = 0; k < blck_size/8; ++k) {
                y[ibl].qs[(blck_size/8)*ib + k] = index[k] & 255;
                h |= (index[k] >> 8) << 3*k;
            }
            y[ibl].qh[ib] = h;
            GGML_ASSERT(scale >= 0);
...
static void quantize_row_iq1_m_impl(const float * GGML_RESTRICT x, void * GGML_RESTRICT vy, int64_t n, const float * GGML_RESTRICT quant_weights,
    ...
    const int blck_size = IQ1M_BLOCK_SIZE;
...
static void quantize_row_iq4_nl_impl(const int super_blck_size, const int blck_size, const float * GGML_RESTRICT x,
    ...
    float sigma2 = 0;
    for (int j = 0; j < super_blck_size; ++j) sigma2 += x[j]*x[j];
    sigma2 *= 2.f/super_blck_size;

    memset(q4, 0, super_blck_size/2);
    dh[0] = GGML_FP32_TO_FP16(0.f);

    float max_scale = 0, amax_scale = 0;
    for (int ib = 0; ib < super_blck_size/blck_size; ++ib) {
        const float * xb = x + ib*blck_size;
        uint8_t * Lb = L + ib*blck_size;
        if (quant_weights) {
            const float * qw = quant_weights + ib*blck_size;
            for (int j = 0; j < blck_size; ++j) weight[j] = qw[j] * sqrtf(sigma2 + xb[j]*xb[j]);
        } else {
            for (int j = 0; j < blck_size; ++j) weight[j] = xb[j]*xb[j];
        }
        float amax = 0, max = 0;
        for (int j = 0; j < blck_size; ++j) {
            float ax = fabsf(xb[j]);
    ...
    if (super_blck_size/blck_size > 1) {
        int nb = super_blck_size/blck_size;
        ...
        if (ntry > 0) {
            ...
            for (int j = 0; j < super_blck_size; ++j) {
                L[j] = best_index_int8(16, values, id*x[j]);
            }
        }
    }

    for (int i = 0; i < super_blck_size/32; ++i) {
        for (int j = 0; j < 16; ++j) {
